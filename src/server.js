...
const getLogin = async () => {
    try {
        const res = await axios.get('https://www1.tln.com.br/apps/ecommerce/autenticacao/Login', {
            headers: {
                'Cookie': `${process.env.COOKIE_REQ_1}`
            }
        })
...
const postLogin = async (__RequestVerificationToken) => {
    ...
    try {
        const res = await axios.post('https://www1.tln.com.br/apps/ecommerce/autenticacao/login', data, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'Cookie': `${process.env.COOKIE_REQ_2}`
            }
        })
        if (res && res.status == 200 && res.data && typeof res.data === 'string') {
            const $ = cheerio.load(res.data);
...
const transaction = async (__AuthorizationToken) => {
    ...
    try {
        const res = await axios.post('https://www1.tln.com.br/apps/ecommerce/transacaolink/index', qs.stringify(data), {
            headers: {
...
const generateLink = async (props) => {
    ...
    try {
        const res = await axios.post('https://www1.tln.com.br/apps/ecommerce/transacaolink/geralink', qs.stringify(data), {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8',
                'Cookie': process.env.COOKIE
            }
...
const runAll = async (props) => {
    ...
    //     // __RequestVerificationToken2 = res4.value
    // } else {
    //     return res4
    // }
